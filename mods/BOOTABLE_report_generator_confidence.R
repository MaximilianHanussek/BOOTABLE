...
scaling_mean_real_times_vector <- c()   # Initialize vector for the mean values of the real times of all replica for all summary files
scaling_all_real_times_vector <- c()    # Initialize vector for the values of the real times of all replica for all summary files
...
for (summary_file in ordered_summary_file_paths){                                # Iterate over the ordered filepaths vector
  ...
  scaling_all_real_times_vector <- c(scaling_all_real_times_vector, real_values_all_vector)          # Collect all the real time values from every summary txt file for scaling functionality
...
if (scaling_flag == "scaling") {                                              # Check if scaling is desired by scaling flag
  ...
  index_extrema <- 0                                                          # Initialize index for min/max value collection
  ######################################################
  ...
  for (i in 1:scaling_number_of_used_tools) {
    ...
    min_value_vector_per_used_core <- c()                                                             # Initialite empty vector for min values
    max_value_vector_per_used_core <- c()                                                             # Initialite empty vector for max values
    ...
    for (core_number in scaling_cores_vector) {
      ...
      for (replica in 1:number_of_used_replica) {
        inter_min_value <- c(inter_min_value, scaling_all_real_times_vector[index_extrema+replica])
        inter_max_value <- c(inter_max_value, scaling_all_real_times_vector[index_extrema+replica])
      }
      min_value_vector_per_used_core <- c(min_value_vector_per_used_core, min(inter_min_value))
      max_value_vector_per_used_core <- c(max_value_vector_per_used_core, max(inter_max_value))
      index <- index + scaling_number_of_used_tools
      index_extrema <- index_extrema + scaling_number_of_used_tools * number_of_used_replica
    }
    index_extrema <- 0
    index_extrema <- index_extrema + number_of_used_replica * i
    ###################################################################################################################
    ### END Iterate over mean real time values vector and pick out the ones corresponding to the correct used cores ###
    ###################################################################################################################
    
    polygon_dataframe <- data.frame(x=scaling_cores_vector, lwr=min_value_vector_per_used_core, upr=max_value_vector_per_used_core)
    ...
    plot(scaling_cores_vector, 
         ...
         ylim = range(c(min(min_value_vector_per_used_core), max(max_value_vector_per_used_core))),
    ...
    with(polygon_dataframe, polygon(c(x,rev(x)),c(lwr,rev(upr)),col = "grey90", border = FALSE))
    
    # Plot line for tool in set up empty plot for the used cores    
    lines(scaling_cores_vector,
          y_value_vector, type = "p", pch = 20, cex = 0.32)
    lines(scaling_cores_vector,
          y_value_vector, lwd = 0.1)
    lines(scaling_cores_vector,
          min_value_vector_per_used_core, col="azure4", lwd = 0.05, lty = 2)
    lines(scaling_cores_vector,
          max_value_vector_per_used_core, col="azure4", lwd = 0.05, lty = 2)
