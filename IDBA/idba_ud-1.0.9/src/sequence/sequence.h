...
public:
    ...
    { Assign(num, ch); }
    explicit Sequence(const CompactSequence &compact_seq, int offset = 0, size_t length = std::string::npos)
    { Assign(compact_seq, offset, length); }
    explicit Sequence(const ShortSequence &short_seq)
    { Assign(short_seq); }
    explicit Sequence(const Kmer &kmer)
    { Assign(kmer); }
    ...
    const Sequence &operator =(const CompactSequence &compact_seq) { Assign(compact_seq); return *this; }
    const Sequence &operator =(const ShortSequence &short_seq) { Assign(short_seq); return *this; }
    const Sequence &operator =(const Kmer &kmer) { Assign(kmer); return *this; }
    ...
    const Sequence &Assign(const CompactSequence &compact_seq, int offset = 0, size_t length = std::string::npos);
    const Sequence &Assign(const ShortSequence &short_seq);
    const Sequence &Assign(const Kmer &kmer);
    ...
    Kmer GetKmer(uint32_t offset, uint32_t kmer_size) const;
