...
{
public:
    explicit HashGraphVertex(const Kmer &kmer = Kmer()): kmer_(kmer) {}
    ...
    const Kmer &key() const { return kmer_; }
    void set_key(const Kmer &key) { kmer_ = key; }

    const Kmer &kmer() const { return kmer_; }
    void set_kmer(const Kmer &kmer) { kmer_ = kmer; }
 ...
 * can make sure the access to vertex consistant.
 */
